<template>
    <div>
        <Card>
            ...
            <div style="height:660px;padding:20px 40px 0 20px;">
                ...
                <Table :loading="subject.loading" border stripe  size="small" :columns="subcolumns" :data="subject.items" width="800" height="360"></Table>
...
<script>
import {VcpColumns,SubjectColumns} from './table/table';
...
export default {
    ...
    methods:{
        ...
        addOneVCP: function(){
            ...
            this.$Modal.confirm({
                ...
                onOk: () => {
                    ...
                    if(commit.name.length === 0 || commit.disp.length === 0){
                        this.$Message.error({content:"数据不全",duration:10});
                        this.$Modal.remove();
                        return;
                    }
                    if(data.teston === true){
                        commit['testval']=data.testval;
                    }
        ...
        charge:function(){
            var vcp = this.data.accounts[this.data.current];
            this.modifyvalue = 1000;
            this.$Modal.confirm({
            render: (h) => {
                return h('div',[
                    h('p',{style:{marginBottom:'30px'}},vcp.accname+'账户充值'),
                    h('div',{style:{margin:'0 10px'}},[
                        h('span',{style:{marginRight:'10px'}},'充值金额'),
                        h('InputNumber',{props: {value: this.modifyvalue,autofocus: true,min:1000,max:10000},
                            on: {input: (val) => {
                                this.modifyvalue = val;
                                }}
                            }),
            ...
            onOk: () => {
                console.log('modifyvalue:'+this.modifyvalue);
                console.log('vcp:'+JSON.stringify(vcp));
                var commit = {apikey:vcp.apikey, charge:this.modifyvalue};
                this.commitVcpInfo(commit);
            }
            })
        },
        testOnOff:function(){
            ...
            onOk: () => {
                ...
                if(this.modifyvalue.on === false && orig.on === false){
                    ...
                    var commit = {apikey:vcp.apikey, teston:this.modifyvalue.on};
                    if(this.modifyvalue.on){
                        commit['testval']=this.modifyvalue.val;
                    }
                    this.commitVcpInfo(commit);
                }
            }
        ...
        commitVcpInfo:function(commit){
            this.$http.post(VURL_VCP_ACCOUNT_UPDATE,commit).then((response) => {
                if(response.body.state === 200000){
